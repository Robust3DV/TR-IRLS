...
#include "robust/robust_lsq_schur_pcg_truncated_lsq.h"
...
std::string method="trunc";
typedef Robust_LSQ_Optimizer_Schur_PCG_TruncLSQ Optimizer_Base;


int const iterationsFactor = 1;
...
struct BundleCostFunction : public NLSQ_CostFunction, public BundleCost_Base
{
   ...
   virtual bool isInlier(int const i)
   {
      ...
      double const inlierThreshold1 = _inlierThreshold;
      /* double const inlierThreshold1 = 0.33*_inlierThreshold; */
                             ...
                             double inlierThreshold)
{
   ...
   opt.maxIterations = 100;// iterationsFactor * nBundleIterations;
   opt.tau = 1e-3;
   //opt.tau = 1e-6;
   ...
   if (kernel != "") method += "_" + kernel;
   std::string out_name = method + "_results.txt";
   ofstream stat_out(out_name);
   for (int i = 0; i < opt._stat_time.size(); i++){
       /* stat_out <<std::setprecision(9)<< opt._stat_time[i] << " " << opt._stat_cost[i] << std::endl; */
       stat_out <<std::setprecision(9)
           << opt._stat_time[i] << " " 
           << opt._stat_cost[i] << " "
           <<opt._stat_inlier_ratio[i] << " " 
           <<opt._stat_truncated_cost[i] << std::endl;
           /* <<0 << std::endl; */
   }
   stat_out.close();
