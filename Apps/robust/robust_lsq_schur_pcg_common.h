...
namespace Robust_LSQ{
    ...
    struct Robust_LSQ_Optimizer_Schur_PCG_Base: public NLSQ_LM_Optimizer{ 
        ...
        void updateStat(double const &current_cost, 
                double const &current_truncated_cost){
            ...
            if (current_truncated_cost < _bestTruncatedCost){
                _bestTruncatedCost = current_truncated_cost;
            }
            ...
            _stat_cost.push_back(_bestCost/_nMeasurements);
            _stat_inlier_ratio.push_back(_bestInlierRatio);
            _stat_truncated_cost.push_back(_bestTruncatedCost/_nMeasurements);
            _timer.start();
        }
        ...
        void minimize()
        {
            ...
            for (int iter = 0; iter < maxIterations; iter++){
                ...
                curr_err = eval_current_cost(cached_errors);
                double const current_truncated_cost = computeTruncatedLSQ(cached_errors[0]);

                std::cout << "IRLS It: " << iter << " err : " << curr_err 
                    << " truncated err: " 
                    << current_truncated_cost
                    << " lambda : " << _damping
                    << std::endl;
                updateStat(curr_err, current_truncated_cost);
        ...
        double clamp(double const x) const {
            return 0.25 * (abs(x) > 1.0 ? 1 : x);
        }

        double computeTruncatedLSQ(Vector<double> const &errors){
            double cost = 0;
        ...
        std::vector<double> _stat_time, _stat_cost, _stat_truncated_cost, _stat_inlier_ratio;
        V3D::Timer _timer;
        double _bestCost, _bestInlierRatio, _bestTruncatedCost;
